...
describe("LocalStorage", () => {
  ...
  describe("security - path traversal prevention", () => {
    ...
    test("rejects key with multiple .. sequences", () => {
      expect(() => storage["resolvePath"]("foo/../../outside")).toThrow(
        "escapes storage"
      );
    });
    ...
    test("allows keys within storage directory", () => {
      ...
      expect(() =>
        storage["resolvePath"]("deeply/nested/path/file.txt")
      ).not.toThrow();
    });
    ...
    test("get rejects path traversal", async () => {
      await expect(storage.get("../outside.txt")).rejects.toThrow(
        "escapes storage"
      );
    });

    test("put rejects path traversal", async () => {
      await expect(storage.put("../outside.txt", "malicious")).rejects.toThrow(
        "escapes storage"
      );
    });
    ...
    test("exists rejects path traversal", async () => {
      await expect(storage.exists("../outside.txt")).rejects.toThrow(
        "escapes storage"
      );
    });

    test("getStream rejects path traversal", async () => {
      await expect(storage.getStream("../outside.txt")).rejects.toThrow(
        "escapes storage"
      );
    });
