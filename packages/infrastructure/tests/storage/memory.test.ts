...
describe("MemoryStorage", () => {
  ...
  describe("metadata", () => {
    ...
    test("sets lastModified timestamp", async () => {
      ...
      const result = await storage.get("test.txt")
      expect(result!.metadata.lastModified!.getTime()).toBeGreaterThanOrEqual(before.getTime())
      expect(result!.metadata.lastModified!.getTime()).toBeLessThanOrEqual(after.getTime())
    })
  })
