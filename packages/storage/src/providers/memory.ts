...
export class MemoryStorageProvider implements Storage.Provider {
  ...
  async getUrl(key: string, _options?: Storage.UrlOptions): Promise<string | null> {
    const normalizedKey = this.normalizeKey(key)
    const stored = this.store.get(normalizedKey)
  ...
  async getUploadUrl(
    _key: string,
    _options?: Storage.UploadUrlOptions
  ): Promise<Storage.UploadCredentials | null> {
    // Memory storage doesn't support direct upload URLs
    return null
  }

  async *list(prefix: string, options?: Storage.ListOptions): AsyncIterable<Storage.Entry> {
